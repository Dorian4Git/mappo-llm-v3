...
However, they are completely blind to the map layout. The environment strictly enforces the Dec-POMDP partial observability (established in Section~\ref{subsec:dec_pomdp}) by completely hiding the coordinates of the static target zones (e.g., resources, workbenches) from the state vector. Because the true mathematical state of the environment includes these target locations, hiding them creates a cognitive "fog of war". This forces the agents to physically explore the spatial grid to locate their objectives, rather than simply calculating a direct mathematical path to a known coordinate. To further prevent the policies from overfitting and memorizing a static sequence of movements, the agents' starting coordinates are slightly randomized at the beginning of every episode.
...
Why Not a Game Engine? Initially, robust open-source game engines such as Godot~\parencite{godot} were considered for the simulation backbone. Godot provides a powerful visual editor, robust collision physics, and a mature node-based hierarchy, making it an optimal choice for traditional game development.

However, modern on-policy deep reinforcement learning algorithms like MAPPO require simulating tens of millions of environment transitions to converge. Full-stack game engines introduce massive computational overhead for rendering pipelines and internal state management. Attempting to spawn hundreds of headless engine instances in parallel quickly bottlenecks system memory and CPU inter-process communication.

As a result, the environment was engineered natively in Python, utilizing NumPy~\parencite{numpy} as the core mathematical engine and Pygame~\parencite{pygame} as a lightweight visual wrapper. This matrix-first approach offers critical architectural advantages for reinforcement learning:
\begin{itemize}
    \item \textbf{Direct Tensor Integration:} The underlying grid, entities, and inventory states exist purely as NumPy matrices. This bypasses the massive serialization and deserialization bottlenecks typically encountered when passing state data between a Python-based RL training script and an external C++ game engine.
    \item \textbf{Seamless NumPy-to-Game Translation:} Because the environment state is natively represented as a matrix, Pygame can trivially translate these numerical coordinates into a visual interface for human debugging. The mathematical logic (kinematics, collisions) is completely decoupled from the rendering loop.
    \item \textbf{Headless Vectorization:} During the actual training loop, the Pygame visual renderer is entirely disabled. The PyTorch models interact directly with the raw NumPy arrays, allowing a vectorized wrapper to step thousands of environments simultaneously in parallel without any graphical or inter-process overhead.
\end{itemize}
