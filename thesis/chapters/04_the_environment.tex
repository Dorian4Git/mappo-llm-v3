...
Before detailing the neuro-symbolic architectures developed in this thesis, this chapter establishes the experimental testbed. The intersection of Large Language Models (LLMs) and Multi-Agent Reinforcement Learning (MARL) requires an environment that exhibits both kinetic complexity (movement, collision avoidance, spatial reasoning) and semantic dependencies (long horizon tasks gated by logical preconditions). To this end, we have engineered a custom 2D multi-agent grid-world environment entirely from scratch.
