...
\begin{enumerate}
    ...
    \item Endowing the LLM with counterfactual reflection delayed convergence across all architectures. For the fine-tuned model (QLoRA), reflection caused a minor delay (converging at 7.08M steps vs.\ 6.29M steps without reflection) because the model developed an unintended bias from its synthetic data: interpreting kinetic delays as logical failures, resulting in $2.3\times$ more staleness resets. However, for the un-finetuned base model, the failure mode was entirely different. The base model converged in 16.32M steps without reflection but took 32.31M steps with reflection. Surprisingly, reflection actually \textit{reduced} staleness resets for the base model (28{,}927 vs.\ 78{,}535). This indicates that rather than thrashing, reflection gave the un-finetuned Commander false confidence to generate executable but incorrect plans (e.g., repeatedly harvesting basic resources). This confirms that reactive self-debugging is counterproductive if the model cannot distinguish between logical errors and physical friction, and becomes actively misleading if the model uses reflection to justify the wrong strategy.
\end{enumerate}
...
Beyond the quantitative metrics, the PPO workers also exhibited emergent cooperative strategies that were not explicitly programmed, such as asymmetric crafting (one agent forges while the other pre-positions near the enemy) and spawn camping (an agent waits on the Gold tile while its partner fights). These behaviors demonstrate a key strength of the neuro-symbolic hierarchy: the LLM prevents agents from getting lost in sparse-reward graphs, while the RL policy independently discovers spatial and temporal optimizations.
