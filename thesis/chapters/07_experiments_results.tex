...
\begin{enumerate}
    ...
    \item \textbf{LLM Dynamic (Gemini 3.1 Flash Lite):} The proposed ``Mixing Board'' architecture, utilizing Google's Gemini 3.1 Flash Lite model via API as the Commander to dynamically shift potential-based reward weights. 
...
The No-LoRA run's subtask curves exhibit a distinctive ``staircase'' pattern (\Cref{fig:hrl_all_subtasks}, Bridge and Enemy panels): long plateaus at 0\% are followed by abrupt jumps to 100\%. This indicates the un-finetuned Commander lacks the granularity to guide workers through incremental progress; instead, it relies on the PPO policy to accidentally discover the correct behavior, after which the completion rate saturates instantly. Both QLoRA runs, by contrast, show smooth, monotonic ramps, evidence of deliberate, structured option sequencing by the fine-tuned Meta-Controller.
...
Conversely, the `Furnace` shift failed catastrophically on advanced tasks (`Iron`, `Sword`) due to reward hacking. The Commander correctly deduced zero-shot that a Furnace serves an identical semantic purpose to a crafting bench. It demonstrated this reasoning in the raw intervention logs:
