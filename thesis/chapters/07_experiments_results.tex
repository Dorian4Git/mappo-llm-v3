...
The key metric validating the LLM Dynamic architecture is the progression through the environment's strict dependency Directed Acyclic Graph (DAG). 
...
A practical consideration when integrating LLMs tightly into the RL training loop is the wall-clock time overhead. Because the meta-controller queries the LLM dynamically throughout the episode, relying on external or cloud-based APIs introduces severe network latency and rate-limiting bottlenecks. 
...
This shift was executed by dynamically injecting an alias mapping into the orchestrator mid-training, forcing the Commander to output weights for the alias rather than the canonical node name, which the orchestrator then reversed and fed to the PPO workers.
...
As seen in Figures~\ref{fig:semantic_shift_subtasks} and~\ref{fig:shift_all_subtasks}, the `Zylorg` control unexpectedly succeeded across almost all subtasks, achieving near-parity with the baseline's extrinsic reward. When the Commander encountered "Zylorg", it lacked a semantic prior. As a result, it assigned it a balanced priority weight alongside other nodes, focusing its reasoning on recognizable end-goals like enemies.
...
This finding highlights a key vulnerability in neuro-symbolic MARL architectures: while LLMs possess exceptional zero-shot semantic mapping, coupling high-confidence oracle weights with dense gradient shaping can inadvertently trap low-level workers in local optima.