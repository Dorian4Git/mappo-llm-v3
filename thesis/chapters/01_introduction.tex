...
\begin{description}
    \item[\textbf{RQ1:}] How can the zero-shot reasoning of Large Language Models dynamically shape potential-based rewards, and does this continuous shaping effectively mitigate exploration bottlenecks in sparse-reward MARL environments?
    \item[\textbf{RQ2:}] Does a Hierarchical Reinforcement Learning (HRL) architecture, where the LLM acts as a discrete Meta-Controller, outperform continuous reward shaping in both sample efficiency and agent coordination for long-horizon tasks?
    \item[\textbf{RQ3:}] What vulnerabilities arise from semantic over-confidence in neuro-symbolic MARL architectures, and how can mathematical constraints like Kullback-Leibler ($D_{KL}$) divergence guardrails stabilize execution policies against non-stationary objective shifts?
\end{description}
...
Specifically, the subject is addressed through two implementations. The first implementation, termed the Mixing Board, relies on dynamic reward shaping. Here, the LLM acts as a strategic meta-supervisor that observes aggregate training statistics to dynamically adjust priority weights over subtasks. These weights govern a continuous Potential-Based Reward Shaping (PBRS) signal \parencite{Ng_Harada_Russell_1999}, which guides the MAPPO agents toward critical bottlenecks while theoretically preserving the optimal policy. The second implementation, the Meta-Controller, transitions to a discrete hierarchical reinforcement learning architecture. Building upon the Options framework \parencite{Sutton_Precup_Singh_1999} and hierarchical meta-controllers \parencite{Kulkarni_Narasimhan_Saeedi_Tenenbaum_2016}, the LLM completely replaces the traditional symbolic planner by issuing temporally extended macro-actions directly to the agents. By augmenting the agents' observation space with one-hot option encodings and enforcing strict Kullback-Leibler (KL) divergence guardrails, this approach allows the agents to master the complete dependency graph and adapt to zero-shot semantic shifts in the environment. Through both architectures, this work demonstrates how the zero-shot planning capabilities of quantized, self-hosted language models (QLoRA) \parencite{Dettmers_Pagnoni_Holtzman_Zettlemoyer_2023} can be integrated into deep reinforcement learning pipelines to solve complex cooperative tasks.
