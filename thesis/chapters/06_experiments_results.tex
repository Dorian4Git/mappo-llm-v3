...
If $D_{\text{KL}}^{\text{approx}}$ exceeds the predefined trust region threshold ($\delta = 0.015$), the gradient updates for the current batch are immediately aborted. To empirically validate the necessity of this mechanism, we log $\max(D_{\text{KL}}^{\text{approx}})$ per update alongside semantic option transitions. 

\begin{figure}[H]
    \centering
    % \includegraphics[width=0.75\textwidth]{plots/hrl/kl_spike_overlay.png}
    \caption{Maximum approximate KL divergence plotted over training updates. The dashed vertical lines represent instances where the LLM Meta-Controller issued a new semantic Option. Notice the immediate divergence spikes correlating with these objective shifts, demonstrating the activation of the early-stopping guardrail to prevent policy collapse.}
    \label{fig:kl_spikes}
\end{figure}
