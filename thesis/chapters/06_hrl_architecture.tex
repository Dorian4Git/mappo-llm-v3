...
The \texttt{IDLE} options are excluded from staleness triggers, because intentional waiting (e.g., the lumberjack waiting while the miner collects stone) is a valid strategy.
...
A critical engineering detail is the use of a custom \texttt{DataCollatorForCompletionOnlyLM}. During standard supervised fine-tuning, the causal language modeling loss is computed over all tokens in the sequence, including the system prompt and user message. This wastes gradient signal on tokens that the model should never generate at inference time. Our custom collator scans each tokenized sequence for the ChatML response delimiter (\texttt{<|im\_start|>assistant\textbackslash n}) and masks all preceding tokens by setting their labels to $-100$. As a result, the cross-entropy loss is computed exclusively over the Commander's actual decision output (the JSON response), ensuring that every gradient update directly improves the quality of option selection rather than memorizing prompt boilerplate.
...
This architecture also effectively acts as an automated curriculum generator (contrasting with the manual Curriculum Learning discussed in Section~\ref{sec:curriculum_learning}). While traditional curriculum learning requires a human engineer to laboriously design and schedule environments of increasing difficulty, our LLM Meta-Controller dynamically orchestrates a skill curriculum on the fly, seamlessly guiding the agents from simple resource gathering to complex combat coordination within a single, static environment.
