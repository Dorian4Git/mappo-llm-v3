"""Regenerate all HRL ablation plots for the 3 clean runs."""
import os, json
import numpy as np
import matplotlib
matplotlib.use('Agg')
...
for task in subtasks:
    ...
    for name in RUNS:
        steps, vals = load_tb(RUNS[name], tag)
        if not steps:
            continue
        vals = np.array(vals)
...
plt.figure(figsize=(8, 5))
for name in RUNS:
    steps, vals = load_tb(RUNS[name], 'Rewards/Avg_Env_Reward')
    if not steps:
        continue
    if len(vals) >= 50:
        s = smooth(vals, 50)
...
plt.figure(figsize=(8, 5))
for name in RUNS:
    steps, vals = load_tb(RUNS[name], 'TD_Error/Abs_Mean')
    if not steps:
        continue
    if len(vals) >= 100:
